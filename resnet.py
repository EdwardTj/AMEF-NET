...
class ResBlk(nn.Module):
    ...
    def __init__(self, ch_in, ch_out,stride=1,):
        ...
        self.conv1 = nn.Conv3d(ch_in, ch_out, kernel_size=(3,3,3), stride=stride, padding=(1,1,1))
        self.bn1 = nn.BatchNorm3d(ch_out)
        self.conv2 = nn.Conv3d(ch_out, ch_out, kernel_size=(3,3,3), stride=(1,1,1), padding=(1,1,1))
...
class Upsample(nn.Module):
    ...
    def __init__(self, ch_in,ch_out):
        ...
        # self.bn1 = nn.BatchNorm3d(ch_out)
        self.conv2 = nn.ConvTranspose3d(ch_in, ch_out, kernel_size=(2,2,2), stride=2)
        self.bn2 = nn.BatchNorm3d(ch_out)
        self.gelu = nn.GELU()
...
class ResNet18(nn.Module):
    ...
    def __init__(self, num_class):
        ...
        self.blk33 = ResBlk(256, 256, stride=1)
        # # [b, 128, h, w, D] => [b, 256, h, w, D]
        self.blk4 = ResBlk(256, 512, stride=2)
        self.blk44 = ResBlk(512, 512, stride=1)
    ...
    def forward(self, x):
        ...
        x1 = self.blk1(x) #(64, 64, 64, 64)
        x1 = self.blk11(x1)
        # print("blk1:", x1.shape)
        x2 = self.blk2(x1) #(128, 32, 32, 32)
        x2 = self.blk22(x2)
        # print("blk2:", x2.shape)
        x3 = self.blk3(x2) #(256, 16, 16, 16)
        x3 = self.blk33(x3)
        # print("blk3:", x3.shape)
        x4 = self.blk4(x3) #(512, 8, 8, 8)
        # x4 = self.blk44(x4)
        print("blk4:", x4.shape)

        # x4,x3,x2上采样为p3,p2,p1
        p3 = self.upsample4(x4) #p3 = x3 (256, 16, 16, 16)
        # print("p3:", p3.shape)
        p2 = self.upsample3(x3) #p2 = x2 (128, 32, 32, 32)
        # print("p2:", p2.shape)
        p1 = self.upsample2(x2) #p1 = x1 (64, 64, 64, 64)
        # print("p1:", p1.shape)

        # x3,x2,x1做eca成 eca_x3,eca_x2,eca_x1,
        x4 = self.eca_x4(x4)
        x4 = self.x4changechannel(x4)  #(512, 8, 8, 8)
        eca_x3 = self.eca_x3(x3)  # (256, 16, 16, 16)
        # print("eca_x3:", eca_x3.shape)
        eca_x2 = self.eca_x2(x2)  # (128, 32, 32, 32)
        # print("eca_x2:", eca_x2.shape)
        eca_x1 = self.eca_x1(x1)  # (64, 64, 64, 64)
        # print("eca_x1:", eca_x1.shape)

        # x4 ( 256,8,8,8)
        p3 = p3+eca_x3  # (256, 16, 16, 16)
        # print("p3:", p3.shape)
        p2 = p2+eca_x2  # (128, 32, 32, 32)
        # print("p2:", p2.shape)
        p1 = p1+eca_x1  # (64, 64, 64, 64)
        # print("p1:", p1.shape)
